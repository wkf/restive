...
module.exports = ->
  ...
  class Project
    wrap = (klass, options) ->
      if klass.__wrapped__
        klass
      else
        _extend _.clone(klass), options
    ...
    Project.tagged('writable', 'required').validate p

    # String.with(length: 5..55, pattern: /1%/).validate p

  # class Schema extends Treeish.Tree
